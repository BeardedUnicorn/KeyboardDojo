 ...
 */
const PracticeHeatmap: React.FC<PracticeHeatmapProps> = ({
  ...
  // Generate practice data (mock data for now)
  const practiceData = useMemo(() => {
    if (!progress || isLoading) return null;
    
    // In a real implementation, this would come from actual user practice history
    ...
    // Generate random practice sessions for the past few weeks
    for (let i = 0; i < weeks * 7; i++) {
      const date = new Date(startDate);
      date.setDate(startDate.getDate() + i);
      
      // Random number of sessions (more likely to have sessions on weekdays)
      const dayOfWeek = date.getDay();
      const isWeekend = dayOfWeek === 0 || dayOfWeek === 6;
      const maxSessions = isWeekend ? 3 : 5;
      const randomFactor = Math.random();
      
      // 30% chance of no practice on a given day
      if (randomFactor > 0.3) {
