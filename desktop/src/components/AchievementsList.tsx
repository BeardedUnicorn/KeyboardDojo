...
import { selectAchievements, selectCompletedAchievements, refreshAchievements } from '../store/slices/achievementsSlice';
...
const AchievementsList: FC<AchievementsListProps> = ({
  ...
  const filteredAchievements = completedAchievements.filter((a) => {
    ...
    const secretMatch = showSecretAchievements || !a.achievement.secret;
              ...
              <CardContent>
                ...
                <Box sx={{ mb: 1, mt: 2 }}>
                  <Box sx={{ display: 'flex', justifyContent: 'space-between', mb: 0.5 }}>
                    ...
                    <Typography variant="caption" color="text.secondary">
                      {
                        Math.min(progress || 0, achievement.condition?.target || 100)
                      }/{achievement.condition?.target || 100}
                    </Typography>
                  </Box>
                  <LinearProgress
                    variant="determinate"
                    value={completed ? 100 : ((progress || 0) / (achievement.condition?.target || 100)) * 100}
