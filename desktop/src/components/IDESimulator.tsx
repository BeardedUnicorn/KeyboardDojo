...
const IDESimulator: FC<IDESimulatorProps> = ({
  ...
  const handleKeyDown = (e: KeyboardEvent<HTMLDivElement>) => {
    ...
    const isCorrect = currentKeysLower.join('+') === expectedLower.join('+');
    ...
    if (isCorrect) {
      ...
      // Award XP and currency for completing the lesson
      addXP(
        XP_REWARDS.COMPLETE_LESSON, 
        'lesson_complete',
      );

      const lessonTitle = lesson?.title || 'IDE Simulator';
      ...
      addCurrency(
        ...
        'perfect_lesson',
        `Perfect completion of lesson: ${lessonTitle}`,
      );
