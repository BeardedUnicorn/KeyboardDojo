...
const CurriculumProgressChart: FC<CurriculumProgressChartProps> = ({
  ...
  const { completedLessons, completedModules, isLoading } = userProgress;
  const theme = useTheme();

  // Calculate progress percentages for each track and module
  const progressData = useMemo(() => {
    if (!tracks || !completedLessons || !completedModules || isLoading) return null;
  ...
  }, [tracks, completedLessons, completedModules, isLoading]);
  ...
  return (
    <Box>
      {progressData.map(
        ({ track, completionPercentage, completedLessons, totalLessons, modules }) => (
          <Paper
            key={track.id}
            elevation={0}
            sx={{
